...
void ERItemRandomiser::RunSaveListener() 
{
	...
	if (ITEM_DEBUG) {
		...
		//wprintf_s(L"dhakjshdkjasdhkjasdhkjasdh\n");
	};
	...
	//tools_client::get_instance().write_log("123321abccba");

	//tools_client temp_tools;
...
//		//wprintf_s(L"Log_1 %ls\n", tempABC.c_str());
//#endif
//
//		RandomiseType randomise_type = static_cast<RandomiseType>(option_reader.GetInteger(header_segment, segment.c_str(), 0));
...
//	};
	...
	HFILE seed_file = OpenFile("ReplayableEdition//random_seed.txt", &file_struct, OF_READWRITE);
	if (seed_file == HFILE_ERROR) {
		...
		HANDLE new_file_creation = CreateFileA("ReplayableEdition//random_seed.txt", GENERIC_READ | GENERIC_WRITE, 0,
